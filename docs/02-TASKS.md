...
### T-002 (done) UI загрузки изображения
...
### T-003 (done) Превью исходного изображения («Before»)
...
### T-004 (done) Заглушка результата и кнопка «Download» (stub)
...
### T-006 (done) Загрузка модели u2netp.onnx
...
### T-008 (done) Препроцессинг: изображение → тензор
...
- Время одного inference ≤ 10 сек (TECH-SPEC)

**Тест-критерии:**
- Подать изображение — получить маску без ошибки
- Замерить время inference — ≤ 10 сек

---

### T-010 (done) Апскейл маски и композиция в PNG
...
### T-011 (done) Постобработка маски (feather / blur)
...
- Inference: ≤ 10 сек
- Полное время обработки одного фото: ≤ 15 сек
